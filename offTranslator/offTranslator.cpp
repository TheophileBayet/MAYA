...
/*
DeclareSimpleCommand(testCmd, PLUGIN_COMPANY, "4.5");
...
MStatus testCmd::doIt(const MArgList&)
{
	...
	MGlobal::displayInfo("Saltations Vogeur");
	return MS::kSuccess;
}
*/
...
{    
    ...
    while (inputfile.getline (buf, maxLineSize)) {
        MString cmdString;
		// Récupérer d'abord les nombres de points / faces / coins, et faire les boucles correspondantes ! 
        cmdString.set(buf);
		// Split la string en différentes coordonnées et les traiter 
